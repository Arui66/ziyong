...
let lzhd = ($.isNode() ? process.env.lzhd : $.getdata("lzhd")) || ""
let lzhdArr = [];
let data = '';
let msg = '';
...
!(async () => {
    if (typeof $request !== "undefined") {
        ...
        if (!(await Envs()))
            ...
            log(`\n=================== 共找到 ${lzhdArr.length} 个账号 ===================`)
            if (debug) {
                log(`【debug】 这是你的全部账号数组:\n ${lzhdArr}`);
            }
            for (let index = 0; index < lzhdArr.length; index++) {
...
'User-Agent': 'Mozilla/5.0 (Linux; Android 13; 21051182C Build/TKQ1.221013.002; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/86.0.4240.99 XWEB/4425 MMWEBSDK/20230202 Safari/537.36 MMWEBID/3637 MicroMessenger/8.0.33.2320(0x2800213B) WeChat/arm64 Weixin Android Tablet NetType/WIFI Language/zh_CN ABI/arm64 miniProgram/wx50282644351869da',
'Content-Type': 'application/json;charset=UTF-8',
'Accept': 'application/json, text/plain, */*',
'X-LF-DXRisk-Source': '5',
'X-LF-DXRisk-Token':' 64036334SwYqoQxNA4kd2v9SpbUBGSDoGchgOeM1',
'X-LF-Channel': 'C2',
...
'Accept-Language': 'en-us,en',
  ...
  data: {"token":lzhd,"channel":"C2","bu_code":"C20400","task_id":"28"}
};
    if (debug) {
            log(`\n【debug】=============== 这是  请求 url ===============`);
...
async function Envs() {
    if (lzhd) {
        if (lzhd.indexOf("@") != -1) {
            lzhd.split("@").forEach((item) => {

                lzhdArr.push(item);
            });
        } else if (lzhd.indexOf("\n") != -1) {
            lzhd.split("\n").forEach((item) => {
                lzhdArr.push(item);
            });
        } else {
